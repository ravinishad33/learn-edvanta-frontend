...
const InstructorDashboard = () => {
  ...
  const stats = [
    { label: 'Total Courses', value: instructorStats?.totalCourses, change:instructorStats?.courseGrowth , icon: ChartBarIcon, color: 'blue' },
    { label: 'Total Students', value: instructorStats?.totalStudents, change: instructorStats?.studentGrowth, icon: UserGroupIcon, color: 'green' },
    { label: 'Total Revenue', value: "₹"+instructorStats?.totalRevenue, change: instructorStats?.revenueGrowthPercent, icon: CurrencyRupeeIcon, color: 'purple' },
    { label: 'Avg Rating', value: instructorStats?.avgRating, change: instructorStats?.ratingGrowth, icon: StarIcon, color: 'yellow' },
  ];
...
console.log(instructorCourse)
  ...
  const courses = [
    { id: 1, title: 'React Masterclass', students: 1245, revenue: '$12,450', rating: 4.9, status: 'active' },
    { id: 2, title: 'Node.js Backend', students: 856, revenue: '$8,560', rating: 4.7, status: 'active' },
    { id: 3, title: 'Python Data Science', students: 357, revenue: '$3,570', rating: 4.8, status: 'draft' },
  ];
          ...
          {stats?.map((stat, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`p-3 bg-${stat.color || [green]}-100 rounded-lg`}>
                  <stat.icon className={`h-8 w-8 text-${stat.color || [grean]}-600`} />
                </div>
              </div>
            </motion.div>
          ))}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 mt-2">
                        ...
                        <span className="text-sm text-gray-600">
                          ...
                          {((course?.price)- (course?.discountedPrice || 0)) * (course?.enrolledStudents?.length || 0)}
                        </span>
                        <span className="text-sm text-gray-600">
            ...
            <div className="mt-8 bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="h-64 flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl">
                <div className="text-center">
                  ...
                  <div className="text-3xl font-bold text-gray-900 mb-2">{instructorStats?.thisMonthRevenuePercent}</div>
                ...
                {recentStudents.map((student) => (
                  <div key={student.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">Enrolled {student.enrolled}</p>
                    </div>
